...
def run(max_steps=64e3, mode='rgb', root='/nfs/bigdisk/kumarak/datasets/charades/Charades_v1_rgb', split='../data/charades.json',
        batch_size=1, load_model='', save_dir='/data/kumarak_temp/charades_temp_feat',
        i3d_in = '/data/kumarak_temp/i3d_in'):
    # setup dataset
    test_transforms = transforms.Compose([videotransforms.CenterCrop(224)])

    dataset = Dataset(split, 'training', root, mode, test_transforms, num=-1, save_dir=save_dir, i3d_in=i3d_in)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True)

    val_dataset = Dataset(split, 'testing', root, mode, test_transforms, num=-1, save_dir=save_dir, i3d_in=i3d_in)
...
if __name__ == '__main__':
    ...
    run(load_model='/data/kumarak_temp/se_temp/super-events-cvpr18/i3d/models/'+args.load_model,
        save_dir='/data/kumarak_temp/charades_temp_feat/'+args.save_dir)
